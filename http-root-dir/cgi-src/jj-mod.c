...
void httprun(int ssock, char * qstring) {
	fssock = fdopen(ssock, "w+");
	if(fssock == NULL)
	{
		perror("fdopen");
	}
    ...
    if(!name[0]) {print_error("you didn't give your name"); return;}
    if(!address[0]) {print_error("you didn't give your address"); return;}
    if(!phone[0]) {print_error("you didn't give your phone number"); return;}
    if((!subs) && (!slims) && (!sides) && (!drinks)) {print_error("you didn't order anything"); return;}
